    ...
    for i, l1 in enumerate(root_nodes):
        ...
        if l1['children']:
            ...
            for j, l2 in enumerate(l1['children']):
                ...
                y_l2 = y_l1 + h_l1 + v_gaps[2] # 1->2 간격 적용
                h_l2 = 1.0
...
with st.sidebar.expander("↕️ 레벨별 수직 간격 (cm)", expanded=True):
...
st.title("📊 WBS 프로 디자이너 (고도화 버전)")
st.info("파일을 업로드하면 실시간 미리보기가 나타납니다. 사이드바에서 각 레벨 사이의 간격을 미세하게 조정해보세요.")
...
if uploaded_file:
    ...
    if raw_data:
        ...
        st.divider()
        if st.button("🚀 최종 디자인으로 PPT 생성", use_container_width=True):
            final_ppt = generate_ppt(layout_data)
            ppt_io = io.BytesIO()
            final_ppt.save(ppt_io)
            ppt_io.seek(0)
            st.download_button("🎁 PPT 파일 다운로드", ppt_io, "Detailed_WBS_Final.pptx")
